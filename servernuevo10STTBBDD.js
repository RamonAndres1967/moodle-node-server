...
const sessions = {}; 
// sessions[ip] = { phase, topic, questionIndex, name }
...
app.post("/chat", async (req, res) => {
  // 🔥 Capturamos firstname, lastname y userId
  ...
  if (used >= SESSION_LIMIT) {
    ...
    const name = sessions[ip]?.name || "there";

    return res.json({
      reply: `I'm sorry ${name}, but we have reached your 5‑minute practice limit for today. 
