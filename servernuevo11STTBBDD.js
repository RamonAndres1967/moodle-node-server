...
// sessions[ip] = { phase, topic, questionIndex, name, userId }
const sessions = {};
...
app.post("/chat", async (req, res) => {
  ...
  // Si la IP no tiene aún userId asociado y nos llega uno, lo fijamos
  if (!sessions[ip].userId && userId) {
    sessions[ip].userId = userId;
    console.log(`🔐 IP ${ip} vinculada a userId ${userId}`);
  }

  // Si ya hay un userId para esa IP y llega uno distinto, consideramos intento de fraude
  if (sessions[ip].userId && userId && sessions[ip].userId !== userId) {
    console.log(`⚠️ Posible fraude: IP ${ip} ya ligada a userId ${sessions[ip].userId} y ha llegado userId ${userId}. Ignorando el nuevo userId.`);
  }

  // userId efectivo (el que realmente vamos a usar)
  ...
  // Guardar nombre en sesión (solo la primera vez o si no había)
  ...
  if (effectiveUserId) {
    ...
    // Modo fallback: si no hay userId, seguimos controlando por IP (no ideal, pero evita romper)
    const { data: usageRow, error: usageError } = await supabase
      .from("usage")
      .select("seconds")
      .eq("ip", ip)
...
app.post("/ttsTime", async (req, res) => {
  ...
  // Enlazamos IP ↔ userId también aquí, por si la primera llamada es ttsTime
  if (!sessions[ip].userId && userId) {
    sessions[ip].userId = userId;
    console.log(`🔐 (ttsTime) IP ${ip} vinculada a userId ${userId}`);
  } else if (sessions[ip].userId && userId && sessions[ip].userId !== userId) {
    console.log(`⚠️ (ttsTime) Posible fraude: IP ${ip} ya ligada a userId ${sessions[ip].userId} y ha llegado userId ${userId}. Ignorando el nuevo userId.`);
  }
  ...
  if (effectiveUserId) {
    ...
    console.log(`🔊 TTS sumado para userId ${effectiveUserId}: +${seconds}s → total ${newTotal}s`);
    ...
    // Fallback: solo IP (no ideal, pero evita reventar si falta userId)
    const { data: usageRow, error: usageError } = await supabase
      .from("usage")
      .select("seconds")
      .eq("ip", ip)
    ...
    console.log(`🔊 (fallback) TTS sumado para IP ${ip}: +${seconds}s → total ${newTotal}s`);
